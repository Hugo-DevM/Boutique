export default function ContactoPage() {
  return (
    <div className="pt-16 min-h-screen bg-white">
      {/* Header */}
      <div className="bg-gradient-to-br from-violet-600 to-purple-700 py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <p className="text-violet-200 text-sm uppercase tracking-widest font-medium mb-3">
            Estamos aquí para ti
          </p>
          <h1
            className="text-4xl lg:text-5xl font-semibold text-white mb-4"
            style={{ fontFamily: "'Cormorant Garamond', serif" }}
          >
            Contáctanos
          </h1>
          <p className="text-violet-200 max-w-md mx-auto">
            Escríbenos por WhatsApp o visítanos en Puerto Vallarta
          </p>
        </div>
      </div>

      {/* Content */}
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          {/* Contact info */}
          <div className="space-y-8">
            <div>
              <h2
                className="text-3xl font-semibold text-gray-900 mb-6"
                style={{ fontFamily: "'Cormorant Garamond', serif" }}
              >
                Información de contacto
              </h2>
              <div className="space-y-6">
                {[
                  {
                    icon: "📍",
                    title: "Ubicación",
                    lines: ["Puerto Vallarta, Jalisco", "México"],
                  },
                  {
                    icon: "📱",
                    title: "WhatsApp",
                    lines: ["322 215 1711"],
                    href: "https://wa.me/523222151711",
                  },
                  {
                    icon: "🕐",
                    title: "Horario",
                    lines: ["Lunes a Domingo", "9:00 am – 8:00 pm"],
                  },
                ].map((item) => (
                  <div key={item.title} className="flex gap-4">
                    <div className="w-12 h-12 bg-violet-50 rounded-xl flex items-center justify-center text-xl border border-violet-100 shrink-0">
                      {item.icon}
                    </div>
                    <div>
                      <p className="text-sm font-semibold text-gray-900 mb-1">
                        {item.title}
                      </p>
                      {item.lines.map((line) =>
                        item.href ? (
                          <a
                            key={line}
                            href={item.href}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="block text-sm text-violet-600 hover:underline"
                          >
                            {line}
                          </a>
                        ) : (
                          <p key={line} className="text-sm text-gray-500">
                            {line}
                          </p>
                        )
                      )}
                    </div>
                  </div>
                ))}
              </div>
            </div>

            {/* WhatsApp CTA */}
            <div className="bg-violet-50 rounded-2xl p-6 border border-violet-100">
              <h3
                className="text-xl font-semibold text-gray-900 mb-2"
                style={{ fontFamily: "'Cormorant Garamond', serif" }}
              >
                Chatea con nosotras
              </h3>
              <p className="text-sm text-gray-500 mb-4">
                La forma más rápida de resolver tus dudas o hacer un pedido.
              </p>
              <a
                href="https://wa.me/523222151711"
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 bg-green-500 hover:bg-green-600 text-white px-6 py-3 rounded-xl font-semibold transition-colors text-sm shadow-md shadow-green-100"
              >
                <svg
                  className="w-5 h-5 fill-current"
                  viewBox="0 0 24 24"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                </svg>
                Abrir WhatsApp
              </a>
            </div>
          </div>

          {/* Visual side */}
          <div className="hidden lg:flex flex-col gap-5">
            <div className="flex-1 bg-gradient-to-br from-violet-100 to-purple-50 rounded-3xl flex items-center justify-center p-10 border border-violet-100">
              <div className="text-center space-y-4">
                <div className="text-7xl">👗</div>
                <h3
                  className="text-3xl font-semibold text-violet-800"
                  style={{ fontFamily: "'Cormorant Garamond', serif" }}
                >
                  Lumière Boutique
                </h3>
                <p className="text-violet-500 text-sm">
                  Puerto Vallarta, México
                </p>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-5">
              {[
                { emoji: "🚚", title: "Envíos rápidos", sub: "A toda la ciudad" },
                { emoji: "💳", title: "Pago seguro", sub: "Tarjeta y transferencia" },
              ].map((item) => (
                <div
                  key={item.title}
                  className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm text-center"
                >
                  <span className="text-3xl">{item.emoji}</span>
                  <p className="text-sm font-semibold text-gray-900 mt-2">
                    {item.title}
                  </p>
                  <p className="text-xs text-gray-400 mt-1">{item.sub}</p>
                </div>
              ))}
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}
